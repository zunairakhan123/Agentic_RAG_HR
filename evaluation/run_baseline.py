...
os.environ["LANGCHAIN_PROJECT"] = "NextBridge_HR_Agent_Baseline"

COMPANY_BASE_URL = "https://relation-creature-tap-bradley.trycloudflare.com/v1"
MODEL_NAME = "qwen3:30b"

# -------------------------------------------------------------------------
# Custom Lightweight Faithfulness Evaluator
# -------------------------------------------------------------------------
...
async def run_custom_faithfulness_batch(eval_llm: ChatOpenAI, responses: list, contexts_list: list) -> list:
    ...
    semaphore = asyncio.Semaphore(2)  # Limits parallel calls to protect local model VRAM
...
# Agent Generation
# -------------------------------------------------------------------------
async def extract_contexts_from_state(state):
    contexts = []
    for msg in state.get("messages", []):
        if getattr(msg, "type", "") == "tool" and getattr(msg, "name", "") == "search_hr_documents":
...
async def generate_predictions(golden_data):
    ...
    async with AsyncSqliteSaver.from_conn_string("hr_agent_memory_eval.db") as memory:
        ...
        for i, item in enumerate(golden_data):
            ...
            config = {"configurable": {"thread_id": f"eval_baseline_{i}"}}
            ...
            contexts_list.append(await extract_contexts_from_state(result_state))
...
def run_baseline_eval():
    ...
    # 3. Standard Ragas Metrics (Excluding Ragas's internal Faithfulness)
    metrics = [
    ...
    os.makedirs("evaluation/results/baseline", exist_ok=True)
    df.to_csv("evaluation/results/baseline/per_question.csv", index=False)
    df.to_json("evaluation/results/baseline/per_question.json", orient="records", indent=4)
    
    summary = df.mean(numeric_only=True).to_dict()
    summary["total_evaluated"] = len(df)
    
    with open("evaluation/results/baseline/summary.json", "w", encoding="utf-8") as f:
